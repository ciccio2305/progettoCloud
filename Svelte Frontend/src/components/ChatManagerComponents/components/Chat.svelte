<script>
    import ProPic from "../../../img/proPic2.png";
    import Message from "./Message.svelte"
    import src from "../../../img/no-comment.png"

    import { currentChat, currentImage } from "../../../services/stores";
    import { Messages } from "../../../services/stores";
    import {msgToSend} from "../../../services/stores.js";
    import {newMessages} from "../../../services/stores.js";
    import {imgToSend} from "../../../services/stores.js";
    import {Token} from "../../../services/stores.js";
    import { lastMsg } from "../../../services/stores";
    import { lastMsgTime } from "../../../services/stores";
    import { lastMsgUser } from "../../../services/stores";
    import { attachment } from "../../../services/stores";
    

    let daysWeek=["Do", "Lu", "Ma", "Me", "Gi", "Ve", "Sa"];
    let name = "";
    let date='';
    let input = "";

    let fileinput;
    let imageToSend;
    
    let newMsg = {
        content: "Null",
        ts: "Null",
        from: "Null"
    };

    let msgToDisplay = []; //array di messaggi

    let newMsgs = [newMsg];

    let token = "";

    currentImage.subscribe(value => {
        let propic = document.getElementById("proPic");
        if(propic != null || propic != undefined)
            propic.style.backgroundImage = "url(" + value + ")"
    });

    Token.subscribe(value => {
        token = value;
    });
    
    currentChat.subscribe(value => {
        console.log(value)
        name = value;
    });

    newMessages.subscribe(value => {
        console.log(value)
        let newest = $newMessages;
        if (newest.length > 0){
            console.log(newest[0]);
            newMsgs = [...newMsgs, newest[0]];
            //scroll to last message
            let ChatContent = document.getElementById("ChatContent");
            if (ChatContent) setTimeout(()=>{ChatContent.scrollTop = ChatContent.scrollHeight}, 1);
            // lastMsg.set(newest[newest.length-1].content);
            // lastMsgTime.set(newest[newest.length-1].ts);
        }
    });

    Messages.subscribe(value => {
        msgToDisplay = [];
        newMsgs = [];
        console.log(value);
        console.log("Messages del cazzo")

        //scroll to last message
        let ChatContent = document.getElementById("ChatContent");
        if (ChatContent) setTimeout(()=>{ChatContent.scrollTop = ChatContent.scrollHeight}, 1);
        for (let i = value.length-1; i >= 0; i--){
            msgToDisplay.push(value[i]);
        }
        console.log("Messaggi: ")
        console.log(msgToDisplay);
        msgToDisplay = [...msgToDisplay];
        if (msgToDisplay.length > 0) {
            lastMsgUser.set(name);
            lastMsg.set(msgToDisplay[msgToDisplay.length-1].content);
            lastMsgTime.set(msgToDisplay[msgToDisplay.length-1].ts);
        }
    });

    function sendMsg(event){
        if(input == "") return;
        if (event == "send") {
            let msg = {
                to: name,
                content: input,
            }
            console.log(msg)
            msgToSend.set(msg);
            
            //mancava il seguente codice + newMsgs=newMsgs;
            let d = new Date();
            newMsg = {
                content: msg.content,
                ts: d.getHours() + ":" + ((d.getMinutes() < 10 ? '0' : '') + d.getMinutes()),
                from: "sender",
                // @ts-ignore
                dayWeek: daysWeek[d.getDay()],
            }

            newMsgs = [...newMsgs, newMsg];
            input = "";
            //scroll to last message
            let ChatContent = document.getElementById("ChatContent");
            if (ChatContent) setTimeout(()=>{ChatContent.scrollTop = ChatContent.scrollHeight}, 1);
            console.log(newMsgs);
        }
        else {
            event.preventDefault();
            event.stopPropagation();
            if (event.keyCode === 13) {
                let msg = {
                    to: name,
                    content: input,
                }
                console.log(msg)
                msgToSend.set(msg);
                
                //mancava il seguente codice + newMsgs=newMsgs;
                let d = new Date();
                newMsg = {
                    content: msg.content,
                    ts: d.getHours() + ":" + ((d.getMinutes() < 10 ? '0' : '') + d.getMinutes()),
                    from: "sender",
                    // @ts-ignore
                    dayWeek: daysWeek[d.getDay()],
                }

                newMsgs = [...newMsgs, newMsg];
                lastMsgUser.set(" ");
                lastMsgUser.set(name);
                console.log("lastMsgUser: " + name);
            
                lastMsg.set(newMsg.content);
                lastMsgTime.set(newMsg.ts);
                input = "";
                //scroll to last message
                let ChatContent = document.getElementById("ChatContent");
                if (ChatContent) setTimeout(()=>{ChatContent.scrollTop = ChatContent.scrollHeight}, 1);
                console.log(newMsgs);

                
            }
        }
    }

    function allega(e){
        
        let allegati = document.getElementById("allegatiDiv");
        console.log("allegati: " + allegati.style.display)

        let style = getComputedStyle(allegati);

        if (style.display == "none"){
            allegati.style.display = "flex";
        } else {
            allegati.style.display = "none";
        }
    }

    const onFileSelected =(e)=>{
        console.log("fileSelect")
        e.stopPropagation();
        e.preventDefault();
        let flag = true;
        var Image = e.target.files[0];
        let reader = new FileReader();
        reader.readAsDataURL(Image);
                
        reader.onload = e => {
            console.log(e.target.result);
            let base64 = e.target.result;
            let tipo = base64.split(';')[0];
            tipo = tipo.split(':')[1];
            base64 = base64.split(',')[1];
            
            let img = {
                name: e.target?.result.name,
                content: base64,
                type: tipo,
                size: e.total
            };
            console.log("tosend: ");
            console.log(img);
            //sendImg(img);
              
            
            if (flag == true && img.size > 200000){
                alert ("il file supera i 200kb, scegli un file più piccolo");
                
                // console.log("sono nel flag")
                // readBinary(Image);
                // flag = false;
            }else{
                let imgSender = document.getElementById("imgSender");
                imgSender.style.display = "flex";
                let image = imgSender.getElementsByTagName("img")[0];

                image.src = "data:" + img.type + ";base64," + img.content;
                imageToSend = img;  
            }
        } 
    }
    var attach;
    function readBinary(image){
        console.log(image)
        let reader = new FileReader();

        reader.readAsArrayBuffer(image);

        reader.onloadend = function(e) {
            console.log("sono nel reader")
            console.log(e.target.result)
            let blob = new Blob([new Uint8Array(this.result)], {type: "image/png"});
            let form = new FormData();
            form.append("file", blob, "blob");
            form.append("id", "10045");

            const request = new XMLHttpRequest();
            request.open("POST", "http://tinode/v0/file/u/")
            request.setRequestHeader("X-Tinode-Apikey", "AQEAAAABAAD_rAp4DJh05a1HAwFT3A6K");
            request.setRequestHeader("X-Tinode-Auth", "Token " + token);
            request.setRequestHeader("X-Tinode-UserAgent", "TinodeWeb/0.17");
            request.send(form); 
            request.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    console.log("response: " + this.responseText);
                    let response = JSON.parse(this.responseText);
                    console.log(response);
                    let urlAttachment = response.ctrl.params.url;
                    console.log(urlAttachment);
                    attach = {
                        "url":  urlAttachment,
                        img: imageToSend
                    }
                    
                    
                }
            }
        }

        
        
    }

    function sendImg(){
        let imgSender = document.getElementById("imgSender");
        imgSender.style.display = "none";
        let image = imgSender.getElementsByTagName("img")[0];
        image.src = "";
        let allegati = document.getElementById("allegatiDiv");
        allegati.style.display = "none";
        
        // if (imageToSend.size > 200000){
        //     attachment.set(attach);
        // } else 
        
        imgToSend.set(imageToSend);
        // console.log(imageToSend);


        let d = new Date();
        newMsg = {
            content: "data:" + imageToSend.type + ";base64," + imageToSend.content,
            ts: d.getHours() + ":" + ((d.getMinutes() < 10 ? '0' : '') + d.getMinutes()),
            from: "sender",
            // @ts-ignore
            dayWeek: daysWeek[d.getDay()],
        }
        console.log(newMsg);

        newMsgs = [...newMsgs, newMsg];
        let ChatContent = document.getElementById("ChatContent");
        if (ChatContent) setTimeout(()=>{ChatContent.scrollTop = ChatContent.scrollHeight}, 1);
    }
</script>

<div id="ChatContainer">
    
    <div id="imgSender">
        <img id = "imagePreview" src="" alt="toSend">
        <div id="imgSend" on:click={sendImg}>
            <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"> 
                <path d="M291.48,449.94A15,15,0,0,1,278,441.5L207.5,296.57,62.57,226.08a15,15,0,0,1,1-27.41L435.48,49.08A15,15,0,0,1,455,68.6L305.4,440.54A15,15,0,0,1,292,449.93Zm-185.38-236,119.18,58a15,15,0,0,1,6.93,6.93l58,119.18L414,90Z"/>
                <path d="M218.72,300.35a15,15,0,0,1-10.6-25.61L430.47,52.39a15,15,0,1,1,21.21,21.22L229.33,296A15,15,0,0,1,218.72,300.35Z"/>
            </svg>
        </div>

    </div>

    <div id = "emptyPlace">
        <img id="empty" src={src} alt="empty">
        <h3> Sembra che non ci sia nessuna chat aperta qui!</h3>
    </div>

    <div id="internal">

        <div id="TopSection">
            <div id="ProPicAndName">
                <div id="proPic"/>
                <div id="name">{name}</div>
            </div>
        

        <!-- <div id="mid">
            <div id="searchBar">
                <div id="searchBarIcon">
                    <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 18C11.775 17.9996 13.4988 17.4054 14.897 16.312L19.293 20.708L20.707 19.294L16.311 14.898C17.405 13.4997 17.9996 11.7754 18 10C18 5.589 14.411 2 10 2C5.589 2 2 5.589 2 10C2 14.411 5.589 18 10 18ZM10 4C13.309 4 16 6.691 16 10C16 13.309 13.309 16 10 16C6.691 16 4 13.309 4 10C4 6.691 6.691 4 10 4Z" fill="black"/>
                        <path d="M11.412 8.58599C11.791 8.96599 12 9.46799 12 9.99999H14C14.0009 9.47442 13.8976 8.95389 13.6961 8.46848C13.4946 7.98307 13.1989 7.54242 12.826 7.17199C11.312 5.65999 8.68701 5.65999 7.17401 7.17199L8.58601 8.58799C9.34601 7.82999 10.656 7.83199 11.412 8.58599Z" fill="black"/>
                    </svg>
                </div>
                <input type="text" id="searchBarInput" placeholder="Cerca..."/>
            </div>
        </div> -->

        <!-- <div id="quickActions">
            <div id="options">
                <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 10C10.9 10 10 10.9 10 12C10 13.1 10.9 14 12 14C13.1 14 14 13.1 14 12C14 10.9 13.1 10 12 10ZM12 4C10.9 4 10 4.9 10 6C10 7.1 10.9 8 12 8C13.1 8 14 7.1 14 6C14 4.9 13.1 4 12 4ZM12 16C10.9 16 10 16.9 10 18C10 19.1 10.9 20 12 20C13.1 20 14 19.1 14 18C14 16.9 13.1 16 12 16Z" fill="black"/>
                </svg>        
            </div>
        </div> -->
    </div>
    
        <div class="interspace"><div class="date">{date}</div></div>

        <div id = "ChatContent">
            {#each msgToDisplay as msg}
                {#if msg.content != "Null"}
                    <Message text={msg.content} time={msg.ts} user={msg.from}/>
                {/if}
            {/each}
            
            
            {#each newMsgs as msgs}
                {#if msgs.content != "Null"}
                    
                    <Message text={msgs.content} time={msgs.ts} user={msgs.from}/>
                {/if}
            {/each}
            <div id = "anchor"></div>
        </div>
    
        <div id="ChatInput">
            <div id="allegati" on:click={(e) =>{allega(e)}}>
                <div id="allegatiDiv">
                    <div on:click={fileinput.click()}>Immagine
                        <input style="display: none;" type="file" accept=".jpg, .jpeg, .png" on:change={(e)=>onFileSelected(e)} bind:this={fileinput}>
                    </div>
                    <div>Documento</div>
                    <div>Video</div>
                </div>
                <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M17.004 5H9C7.162 5 5.414 5.737 4.076 7.076C2.737 8.415 2 10.163 2 12C2 13.838 2.737 15.586 4.076 16.924C5.414 18.263 7.162 19 9 19H17V17H9C7.697 17 6.45 16.471 5.49 15.51C4.529 14.55 4 13.303 4 12C4 10.698 4.529 9.451 5.49 8.49C6.45 7.529 7.697 7 9 7H17H17.004C17.794 7 18.543 7.314 19.113 7.886C19.684 8.457 19.999 9.208 20 10.002C19.9997 10.3947 19.9214 10.7835 19.7696 11.1457C19.6179 11.5079 19.3957 11.8363 19.116 12.112C18.8392 12.3922 18.5098 12.6148 18.1466 12.7672C17.7835 12.9196 17.3938 12.9987 17 13H9C8.86963 12.9984 8.74086 12.9711 8.62108 12.9196C8.50129 12.8681 8.39286 12.7935 8.302 12.7C8.20794 12.609 8.13283 12.5003 8.08099 12.3802C8.02916 12.2601 8.00163 12.1308 8 12C8 11.748 8.11 11.493 8.301 11.302C8.39171 11.2079 8.50021 11.1327 8.6202 11.0809C8.7402 11.029 8.86929 11.0015 9 11H17V9H9C8.21 9 7.459 9.315 6.886 9.889C6.314 10.461 6 11.211 6 12C6 12.789 6.314 13.54 6.888 14.114C7.16401 14.3939 7.49277 14.6164 7.85529 14.7685C8.21781 14.9205 8.60688 14.9992 9 15H17.001C17.6572 14.9995 18.3068 14.8691 18.9123 14.6163C19.5178 14.3635 20.0673 13.9933 20.529 13.527C20.9948 13.0652 21.3644 12.5156 21.6167 11.9101C21.8689 11.3047 21.9988 10.6552 21.9987 9.99929C21.9986 9.34337 21.8686 8.69395 21.6161 8.08854C21.3637 7.48313 20.9939 6.93371 20.528 6.472C20.0674 6.0055 19.5186 5.63515 18.9137 5.38246C18.3087 5.12977 17.6596 4.99976 17.004 5Z" fill="black"/>
                </svg>
            </div>

            <!-- <div id="emoji">
                <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 2C6.486 2 2 6.486 2 12C2 17.514 6.486 22 12 22C17.514 22 22 17.514 22 12C22 6.486 17.514 2 12 2ZM12 20C7.589 20 4 16.411 4 12C4 7.589 7.589 4 12 4C16.411 4 20 7.589 20 12C20 16.411 16.411 20 12 20Z" fill="black"/>
                    <path d="M14.829 14.828C14.4635 15.1923 14.0317 15.4835 13.557 15.686C12.7023 16.0468 11.7488 16.0995 10.8595 15.835C9.97029 15.5705 9.20056 15.0053 8.682 14.236L7.024 15.355C7.45644 15.9931 8.0066 16.543 8.645 16.975C9.29465 17.4143 10.0246 17.7212 10.793 17.878C11.5898 18.0405 12.4112 18.0405 13.208 17.878C13.9763 17.7209 14.7062 17.4141 15.356 16.975C15.669 16.763 15.968 16.517 16.242 16.244C16.514 15.973 16.762 15.673 16.976 15.355L15.318 14.236C15.1743 14.4485 15.0105 14.6467 14.829 14.828Z" fill="black"/>
                    <path d="M8.5 12C9.32843 12 10 11.3284 10 10.5C10 9.67157 9.32843 9 8.5 9C7.67157 9 7 9.67157 7 10.5C7 11.3284 7.67157 12 8.5 12Z" fill="black"/>
                    <path d="M15.493 11.986C16.3176 11.986 16.986 11.3176 16.986 10.493C16.986 9.66844 16.3176 9 15.493 9C14.6684 9 14 9.66844 14 10.493C14 11.3176 14.6684 11.986 15.493 11.986Z" fill="black"/>
                </svg>
            </div> -->
            <input type="text" placeholder="Scrivi qui..." name="input" on:keyup|preventDefault={sendMsg} bind:value={input}>
            <!-- <div id="vocals">
                <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M16 12V6C16 3.783 14.215 1.979 12.021 1.979C11.9506 1.97942 11.8805 1.98781 11.812 2.004C10.7853 2.05378 9.81693 2.49635 9.10738 3.24015C8.39783 3.98395 8.00136 4.97204 8 6V12C8 14.206 9.794 16 12 16C14.206 16 16 14.206 16 12ZM10 12V6C10 4.897 10.897 4 12 4C12.0547 4.00002 12.1092 3.995 12.163 3.985C13.188 4.06 14 4.935 14 6V12C14 13.103 13.103 14 12 14C10.897 14 10 13.103 10 12Z" fill="black"/>
                    <path d="M6 12H4C4 16.072 7.061 19.436 11 19.931V22H13V19.931C16.939 19.436 20 16.073 20 12H18C18 15.309 15.309 18 12 18C8.691 18 6 15.309 6 12Z" fill="black"/>
                </svg>
            </div> -->
            <div id="send" on:click={() => { sendMsg("send") }}>
                <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"> 
                    <path d="M291.48,449.94A15,15,0,0,1,278,441.5L207.5,296.57,62.57,226.08a15,15,0,0,1,1-27.41L435.48,49.08A15,15,0,0,1,455,68.6L305.4,440.54A15,15,0,0,1,292,449.93Zm-185.38-236,119.18,58a15,15,0,0,1,6.93,6.93l58,119.18L414,90Z"/>
                    <path d="M218.72,300.35a15,15,0,0,1-10.6-25.61L430.47,52.39a15,15,0,1,1,21.21,21.22L229.33,296A15,15,0,0,1,218.72,300.35Z"/>
                </svg>
            </div>
        </div>

    </div>

</div>




<style>
    #ChatContainer {
        position: relative;
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: space-evenly;
        top: 50%;
        transform: translateY(-50%);
        width: 75%;
        min-width: 800px;
        min-height: 700px;
        height: 95%;
        float: left;
        border-radius: 20px;
        margin-left: 20px;
        margin-right: 50px;
        background-color: white;
       
    }

    #emptyPlace{
        position: relative;
        width: 50%;
        height: 50%;
        display: flex;
        align-items: center;
        justify-content: center;
        flex-direction: column;

    }

    #emptyPlace>h3{
        color: rgb(78, 78, 78);
        font-size: 2vh;
    }

    #empty{
        width: 25%;
    }

    #internal{
        display: none;
        flex-direction: column;
        align-items: center;
        justify-content: space-evenly;
        width: 100%;
        height: 100%;
        
    }

    #TopSection {
        height: 70px;
        width: 95%;
        border-radius: 20px;
        background: #e3e3e3;
        display: flex;
        justify-content: space-between;

    }

    #ProPicAndName {
        width: auto;
        min-width: 200px;
        height: 100%;
        display: flex;
        align-items: center;
        justify-content: space-evenly;
        gap: 10px;
        
    }

    #proPic {
        position: relative;
        height: 64px;
        width: 64px;
        border-radius: 100%;
        background-size: cover;
        margin-left: 10px;
    }

    #name {
        position: relative;
        width: auto;
        font-weight: bold;
    }

    #mid{
        width: calc(100% - 70px);
        height: 100%;
        border-radius: 15px;
        display: flex;
        align-items: center;
        justify-content: flex-end;
        
        
    }

    #searchBar{
        min-width: 200px;
        width: 50%;
        height: 50%;
        background-color: #e3e3e3;
        border-radius: 15px;
        display: flex;
        align-items: center;
        box-shadow: 0px 2px 5px 0px rgba(0,0,0,0.75);
        margin-right: 20px;
        background-color: white;
    }

    #searchBarIcon{
        width: 35px;
        height: 35px;
        float: left;
        display:flex;
        align-items: center;
        justify-content: center;
        
        padding: 5px;
       
    }

    #searchBar input{
        width: calc(100% - 35px);
        height: 100%;
        border: none;
        background-color: white;
        border-radius: 15px;
        float: left;
        font-weight: 100;
        font-size: 17px;
        outline: none;
        
    }

    #quickActions{
        width: 70px;
        height: 100%;
        display: flex;
        align-items: center;
        justify-content: flex-end;
        
    }

    #quickActions svg, #ChatInput svg{
        width: auto;
        height: 40px;
        background-color: white;
        border-radius: 100%;
        padding: 5px;
        
    }



    #options {

        width: auto;
        height: 40px;
        margin-right: 10px;
    }

    .interspace {
        width: 100%;
        height: 30px;
        display: flex;
        justify-content: center;
        
    }

    .date {
        font-weight: bold;
        font-size: 20px;
    }

    #ChatContent {
        width: 90%;
        overflow-y: scroll;
        height: calc(100% - 270px);
        scrollbar-width: thin;
        display: flex;
        flex-direction: column;
        
    }

    #ChatContent::-webkit-scrollbar {
      width: 8px;
      background-color: none;
      border-radius: 20px;
    }

    #ChatContent::-webkit-scrollbar-thumb {
      background-color: rgb(155, 155, 155);
      border-radius: 20px;
    }

    #ChatContent::-webkit-scrollbar-track-piece:end {
      background: transparent;
      margin-bottom: 15px;
    }

    #ChatContent::-webkit-scrollbar-track-piece:start {
      background: transparent;
      margin-top: 15px;
    }

    #ChatInput{
        
        width: 90%;
        height: 60px;
        background-color: #e3e3e3;
        border-radius: 15px;
        display: flex;
        align-items: center;
        justify-content: space-around;
    
    }

    input{
        width: 80%;
        height: 80%;
        border-radius: 15px;
        border: none;
        outline: none;
        padding-left: 10px;
        font-size: 15px;
        background-color: #e3e3e3;
    }

    #allegati{
        width: auto;
        height: 30px;
        border-radius: 100%;
        transform: rotate(45deg);
        /* padding: 8px; */
        margin-left: 5px; 
        display:flex;
        align-items: center;
        justify-content: center;
    }

    #allegatiDiv{
        width: 200px;
        height: 150px;
        transform: rotate(-45deg) translate(40%, -75%);
        margin-left: 5px;

        display: none;
        flex-direction: column;
        align-items: center;
        justify-content: space-around;
        position: absolute;
        
        background-color: #e3e3e3;
        border-radius: 15px;
        overflow: hidden;


    }

    #allegatiDiv > div{
        width: 100%;
        height: calc(100% / 3);
        text-align: center;
        line-height: 5vh;
        
    }

    #allegatiDiv > div:hover{
        width: 100%;
        height: calc(100% / 3);
        text-align: center;
        line-height: 5vh;
        background-color: #cfcfcf;
    }

    #emoji {
        width: auto;
        height: 30px;
        border-radius: 100%;
        /* padding: 8px; */
        margin-left: 5px;
        display:flex;
        align-items: center;
        justify-content: center;
        /* border:1px solid red; */
    }

    #vocals {
        width: auto;
        height: 30px;
        border-radius: 100%;
        /* padding: 8px; */
        margin-left: 5px;
        margin-right: 5px;
        display:flex;
        align-items: center;
        justify-content: center;
        /* border:1px solid red; */
    }

    #send {
        width: auto;
        height: 30px;
        border-radius: 100%;
        /* padding: 8px; */
        margin-left: 5px;
        margin-right: 5px;
        display:flex;
        align-items: center;
        justify-content: center;
        /* border:1px solid red; */
    }

    #allegati>svg, #emoji>svg, #vocals>svg, #send>svg {
        width:auto;
        height: 100%;
        border-radius: 100%;
    }

    #quickActions svg:active, #ChatInput svg:active {
        background-color: #cfcfcf;
    }

    #imgSender{
        width: 100%;
        height: 100%;
        display: none;
        flex-direction: column;
        align-items: center;
        justify-content: center;
        position: absolute;
        top: 0;
        left: 0;
        backdrop-filter: blur(5px);
        background-color: #e3e3e372;
        z-index: 999;
        
    }

    #imagePreview{
        max-width: 50vw;
        max-height: 70vh;
        border-radius: 20px;
    }

    #imgSend{
        width: 40px;
        height: 40px;
        position: absolute;
        bottom: 10px;
        right: 10px;
        cursor: pointer;
        
    }
</style>