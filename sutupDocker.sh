...
# Add Docker's official GPG key and repo (Debian vs Ubuntu)
sudo apt-get update
sudo apt-get install ca-certificates curl -y
sudo install -m 0755 -d /etc/apt/keyrings

# Use Debian repo on Debian, Ubuntu repo on Ubuntu (GCE può essere Debian!)
source /etc/os-release
if [ "$ID" = "debian" ]; then
  DOCKER_DISTRO="debian"
  echo "Rilevato Debian: uso repository Docker per Debian ($VERSION_CODENAME)"
else
  DOCKER_DISTRO="ubuntu"
  echo "Rilevato Ubuntu/altro: uso repository Docker per Ubuntu ($VERSION_CODENAME)"
fi
